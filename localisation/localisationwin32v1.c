 ...
 *
 * La version localisationwin32v1.exe est sous le même code
 ...
 */
 ...
 /* ===========================================================================
    compilation : gcc localisationwin32v1.c -o localisationwin32v1.exe -lgdi32
    =========================================================================== */
...
// AsyncStatus = MyAsyncStatus

typedef enum MyAsyncStatus {
    MyAsyncStatus_Started   = 0,
    MyAsyncStatus_Completed = 1,
    MyAsyncStatus_Canceled  = 2,
    MyAsyncStatus_Error     = 3
} MyAsyncStatus;
...
static HRESULT wait_access_debug(IAsyncOperationAccess *operation, GeolocationAccessStatus *result) {
    ...
    printf("[wait_access] Debut de l'attente...\n");
    ...
    printf("[wait_access] QueryInterface(IAsyncInfo) = 0x%08lX\n", (unsigned long)hr);
    ...
    for (;;) {
        ...
        if (FAILED(hr)) {
            printf("[wait_access] get_Status = 0x%08lX\n", (unsigned long)hr);

            goto wait_cleanup;
        }

        printf("[wait_access] status = %d\n", (int)status);
    ...
    printf("[wait_access] Appel GetResults...\n");

    hr = operation->lpVtbl->GetResults(operation, result);

    printf("[wait_access] GetResults = 0x%08lX, result = %d\n", (unsigned long)hr, result ? (int)*result : -1);
...
DWORD WINAPI UiThreadProc(LPVOID lpParam) {
    ...
    if (!dll) {
        printf("LoadLibrary(combase.dll) : %lu\n", GetLastError());
        return 1;
    }
    ...
    {
        printf("Fonction manquante dans combase.dll\n");
        FreeLibrary(dll);
        return 1;
    }
    printf("combase.dll OK\n");
    ...
    printf("WindowsCreateString = 0x%08lX\n", (unsigned long)hr);
    ...
    printf("RequestAccessAsync = 0x%08lX\n", (unsigned long)hr);
    ...
    printf("RequestAccessAsync termine : 0x%08lX\n", (unsigned long)hr);

    if (FAILED(hr))
        goto cleanup;

    printf("GeolocationAccessStatus = %d\n", (int)accessStatus);

    if (accessStatus != GeolocationAccessStatus_Allowed) {
        printf("Acces à la localisation REFUSE.\n");
        printf("Active la localisation dans les paramètres Windows si la machine l'accepte, bouton emplacement.\n");
...

    {
        ...
        printf("ActivationFactory = 0x%08lX\n", (unsigned long)hr);
        ...
        printf("QueryInterface(IGeolocator) = 0x%08lX\n", (unsigned long)hr);
    ...
    printf("GetGeopositionAsync = 0x%08lX\n", (unsigned long)hr);
    ...
    printf("GetGeopositionAsync termine : 0x%08lX\n", (unsigned long)hr);
...
int main(void){
    ...
    HANDLE hThread = CreateThread(NULL, 0, UiThreadProc, &result, 0, NULL);

    WaitForSingleObject(hThread, INFINITE);
    CloseHandle(hThread);

    if (FAILED(result.hr)) {
        printf("Erreur WinRT : 0x%08lX\n", (unsigned long)result.hr);
        Sleep(10000);
        return 1;
    }
